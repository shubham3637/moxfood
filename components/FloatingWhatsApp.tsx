...
export default function FloatingWhatsApp() {
  ...
  // Dynamically position button ABOVE the floating cart bar when items are in cart
  ...
  return (
    <a
      ...
      className={`fixed z-50 group flex items-center justify-center bg-[#25D366] hover:bg-[#20ba5a] text-white p-3.5 sm:p-4 rounded-full shadow-2xl shadow-emerald-600/50 hover:scale-110 active:scale-95 transition-all duration-300 cursor-pointer ${
        hasCartItems
          ? 'bottom-24 right-4 sm:bottom-24 sm:right-6'
          : 'bottom-6 right-4 sm:bottom-6 sm:right-6'
      }`}
      aria-label="Chat on WhatsApp"
      title={language === 'gu' ? 'વોટ્સએપ પર વાતચીત કરો' : 'Chat with Moxfood on WhatsApp'}
    >
